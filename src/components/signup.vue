<template>
  <div class="container">
    ...
    <form @submit.prevent="handleSubmit">
      ...
      <router-link class="back-button" to="/">Back</router-link> <!-- Redirecciona a index.vue -->
    </form>
...
export default {
  ...
  data() {
    return {
      ...
      showModal: false,
      modalMessage: '',
      redirectToHome: false, // Propiedad para redirigir
    };
  },
  methods: {
    async handleSubmit() {
      ...
      try {
        const response = await fetch('http://localhost:5000/api/v1/users/createuser', {
          method: 'POST',
        ...
        if (response.ok) {
          ...
          this.redirectToHome = true; // Indicar que se debe redirigir al cerrar el modal
        } else {
